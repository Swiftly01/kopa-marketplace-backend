...
import { IEmailProvider } from './providers/i-email-provider.interface';
...
export class EmailService {
  ...
  constructor(
    ...
    this.appUrl = configService.get('FRONTEND_URL', 'http://localhost:3000');
    this.appName = configService.get('APP_NAME', 'Kopa Marketplace');
  }
  ...
  <body style="font-family: Arial, sans-serif; color: #333;">
    ...
    <p>
      <a href="${verificationLink}"
         style="display:inline-block;background-color:#007bff;color:#ffffff;padding:10px 20px;text-decoration:none;border-radius:5px;font-weight:bold;">
        Verify Email
      </a>
    </p>
...
`.trim();
    ...
    await this.sendEmail(email, 'Email Verification', textContent, htmlContent);
  }
...
Kopa Marketplace Team
  ...
  private async sendEmail(
    ...
    const id = await this.emailProvider.sendEmail({
      to,
      subject,
      text,
      html,
